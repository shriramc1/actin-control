...
for run_num in range(init_run_num*20, (init_run_num+1)*20):
    ...
    layered_tag += "_".join(map(str, target_observables)) + "_target_observables"
    ...
    for (layer_num, target_observable) in enumerate(target_observables):
        curr_state = cdql.make_layered_by_height(layer_num=layer_num,
